...
Якщо раптом мікроконтролер перестав посилати запити, але підключається до мережі і веб-інтерфейс доступний — перевірте налаштування з'єднання з API:
- За замовчуванням прошивка підключається по HTTPS без перевірки fingerprint, тому зміна сертифіката на сервері не повинна зупиняти роботу.
- За бажанням ви можете вказати свій SSL fingerprint у веб-інтерфейсі на сторінці «Налаштування Channel Key». Тоді прошивка виконуватиме жорстку перевірку сертифіката (пінінг). У разі зміни сертифіката його fingerprint потрібно оновити вручну.
- Також у тій же формі є опція «Використовувати HTTP (небезпечно)». Якщо увімкнути — пристрій відправлятиме запити по HTTP. Ця опція не рекомендується з точки зору безпеки, використовуйте лише за необхідності.
- Де перевірити fingerprint: https://iplocation.io/ssl-certificate-fingerprint
...
- [x] Можливість змінювати Fingerprint сертифікату сервера API
